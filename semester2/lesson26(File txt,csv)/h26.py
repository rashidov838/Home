...
with open('information.csv','w',newline='',encoding='utf-8') as information:
    fields=['name','email','password','purchases','card']
    information_writer=csv.DictWriter(information,fieldnames=fields)
    information_writer.writeheader()
    for user in USERS:
        information_writer.writerow(user)
information.close()